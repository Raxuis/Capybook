...
const LibraryCard = ({ userBook, userId, openBookModal }: LibraryCardProps) => {
  ...
  const isBookFinished = (userBook: any) => {
    return (
  ...
  const handleReviewClick = () => {
    console.log("handleReviewClick", userBook.Book);
    ...
    <Card className="overflow-hidden transition-all hover:shadow-md group">
      ...
      <CardContent className="p-4 pt-3 space-y-4">
        <div className="flex items-center justify-between">
          ...
          {isBookFinished(userBook) && (
            <div className="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-2 items-end sm:items-center">
              <Badge
                variant="outline"
