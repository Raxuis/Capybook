...
const BookNotesModal = ({book, isOpen, setIsOpen, userId}: BookNotesProps) => {
    ...
    }, [sortBy, searchTerm, selectedTags, userId, book.id, toast]);

    useEffect(() => {
        if (isOpen && book.id) {
            fetchNotes();
        }
    }, [isOpen, book.id, fetchNotes]);
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3">
                        ...
                        <AnimatePresence>
                            ...
                            {filteredAndSortedNotes.map((note) => (
                                ...
                                >
                                    <Card
                                        className="hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-white to-slate-50/50 border-slate-200/60 overflow-hidden group">
                                        <CardHeader className="pb-4">
                                            <div className="flex items-start justify-between gap-4">
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                                                        <Badge
                                                            className={`text-xs font-medium shadow-sm ${getTypeColor(note.type as BookNoteType)}`}>
                                                            {getTypeIcon(note.type as BookNoteType)}
                                                            <span className="ml-1.5">{note.type}</span>
                                                        </Badge>
                                                        {note.page && (
                                                            <Badge variant="outline"
                                                                   className="text-xs bg-white/80 border-slate-200">
                                                                Page {note.page}
                                                            </Badge>
                                                        )}
                                                    </div>
                                                ...
                                                <Form {...editForm}>
                                                    <form onSubmit={editForm.handleSubmit(onEditSubmit)}
                                                        ...
                                                        <FormField
                                                            ...
                                                            render={({field}) => (
                                                                <FormItem>
                                                                    <FormLabel className="text-slate-700 font-medium">Type
                                                                        de note</FormLabel>
                                                                    <Select onValueChange={field.onChange}
                                                                            value={field.value}>
                                                                        <FormControl>
