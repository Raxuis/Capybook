        ...
        <section ref={featuresRef} id="fonctionnalités" className="bg-background py-20">
            <div className="container mx-auto px-4">
                ...
                >
                    ...
                    <motion.p className="text-muted-foreground mx-auto max-w-2xl text-xl" variants={animations.fadeInUp}>
                    ...
                    >
                        <TabsList className="bg-muted grid size-full max-w-2xl grid-cols-2 p-2 md:grid-cols-4">
                            {features.map(({ key, label }) => (
                                <motion.div key={key} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex items-center justify-center">
                                        ...
                                        <TabsContent value={key}>
                                            <div className="grid place-items-center gap-8 md:grid-cols-2">
                                                <div className="space-y-4 justify-self-start">
                                                    <motion.div className="bg-primary/10 inline-block rounded-2xl p-3" whileHover={{ scale: 1.1, rotate: 5 }} transition={{ type: "spring", stiffness: 300 }}>
                                                        {icon}
                                                    </motion.div>
                                                    ...
                                                    <motion.ul className="space-y-2" variants={animations.staggerChildren} initial="hidden" animate="visible">
                                                        {points.map((item, index) => (
                                                            <motion.li key={item} className="flex items-center gap-2" variants={{ hidden: { opacity: 0, x: -20 }, visible: { opacity: 1, x: 0, transition: { delay: index * 0.1, duration: 0.5 } } }}>
                                                                <motion.div className="bg-secondary size-2 rounded-full" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3 + index * 0.1 }}></motion.div>
                                                                <span>{item}</span>
                                                            </motion.li>
                                                        ))}
                                                    </motion.ul>
                                                </div>
                                                <motion.div className="bg-muted flex aspect-square w-1/2 items-center justify-center rounded-2xl p-6 md:justify-self-end" animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} variants={animations.cardHover} initial="rest" whileHover="hover">
                                                    <Image src={image} alt={title} width={400} height={400} className="size-full rounded-xl object-cover" />
                                                </motion.div>
                                            </div>
                                        </TabsContent>
                                    </motion.div>
